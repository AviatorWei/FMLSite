...
$sql="SELECT * FROM current WHERE ".$type." LIKE '%".$str."%' ORDER BY Number LIMIT 50";
...
if(mysqli_num_rows($result)>0){
	...
	while($row=mysqli_fetch_assoc($result)){
		...
		echo("</td>"."<td>".$row['Pos']."</td>"."<td>".$row['Club']."</td>"."<td>".$row['Number']."</td>"."<td>".$row['KeyinFML']."</td>"."<td>".$row['Team']."</td>");
...
$sql="SELECT * FROM currentFMC WHERE ".$type." LIKE '%".$str."%' ORDER BY Number LIMIT 50";
$result=mysqli_query($conn,$sql);
echo("in FMC:");
//在一个表格中打印所有符合条件的球员
if(mysqli_num_rows($result)>0){
