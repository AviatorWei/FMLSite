...
elseif($round%2==1){//淘汰赛首回合
	...
	for($i=1;$i<8;$i+=2){
		...
		elseif($array1['firstleggoal']+$array1['tmpGoal']==$array2['tmpGoal']+$array2['firstleggoal']){
			echo("".$array1['pkturn']."\n");
                        echo("".(sizeof($array2['pkturn']))."".$array2['pkturn']."\n");
			$array1['pkturn'] = explode(' ', strtolower($array1['pkturn']));
			$array2['pkturn'] = explode(' ', strtolower($array2['pkturn']));
			//echo("".(sizeof($array1['pkturn'])."\n");
                        //echo("".(sizeof($array2['pkturn']))."".$array2['pkturn']."\n");
			if(sizeof($array1['pkturn']) < 10)
			{
				$array1['pkturn'] = array_reverse(explode(' ',str_replace("\"","",str_replace("/", " ", strtolower($array1["Lineup"])))));
			}
			if(sizeof($array2['pkturn']) < 10)
			{
				$array2['pkturn'] = array_reverse(explode(' ',str_replace("\"","",str_replace("/", " ", strtolower($array2["Lineup"])))));
			}
			echo("".implode(" ", $array1['pkturn'])."\n");
			echo("".implode(" ", $array2['pkturn'])."\n");
			if(!PK($conn,$array1['pkturn'],$array2['pkturn'])){
			echo("Team1 won by PK");
			//mysqli_query($conn,"UPDATE teamsFMC SET isalive=0 WHERE tmpCode=".$i);
			//mysqli_query($conn,"UPDATE teamsFMC SET weight=1 WHERE tmpCode=".($i+1));
			}
			else{
			echo("Team2 won by PK");
