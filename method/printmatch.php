...
function printBroadcast($conn,$name){
	echo("<!DOCTYPE html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0, minimum-scale=1.0, user-scalable=yes'/><title>".$name."</title><style type='text/css'>
	.title
		{
			width: 100vw;
			font-size: 1.8vw;
	...
	</style></head><body><h2 class='title'>一线队</h2>");
//开始输出一线队直播帖
		doPrintBroadcast($conn,"",16,"Teamrank");
		//开始输出首发阵容
		echo("<h2 class='title'>首发阵容</h2>");
		printLineups($conn,"",16);
		//开始输出预备队比分，格式和一线队完全相同，除了队名小写以外
		echo("<h2 class='title'>预备队</h2>");
...
function printpk($conn,$teamnum){
	for($i=1;$i<$teamnum;$i+=2){
		...
		if($array1['firstleggoal']+$array1['tmpGoal']==$array2['firstleggoal']+$array2['tmpGoal'] && $array1['firstleggoal']==$array2['tmpGoal']){
...
function printpklineup($info1){
	echo("<div class='live'><pre>PK: ");
	echo($info1['pkturn']);
	echo("</pre></div>");
}
function printother($conn,$item){
	echo("<div class='title'><pre>".$item."</pre></div>");
...
function printotherscored($conn){
	echo("<div class='title'><pre>Free Agents</pre></div>");
	...
	echo("<div class='title'><pre>Miss</pre></div>");
