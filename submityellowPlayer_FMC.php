...
elseif($num!=$tmpyellow+1){
	echo("输入的黄牌数不对。");
}
//没问题，则开始更新数据库
else{
        $tmpassist=mysqli_fetch_assoc(mysqli_query($conn,"SELECT tmpassist FROM currentFMC WHERE Name='".$str."'"))['tmpyellow']+1;
		mysqli_query($conn,"UPDATE currentFMC SET tmpyellow=".$tmpassist." WHERE Name='".$str."'");
		writeLog("Add ".$str."'s yellow card");
		echo("已添加".$str."的黄牌");
}
mysqli_close($conn);
/*}
