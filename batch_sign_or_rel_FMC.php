...
if(checkCookie()){
    $data=$_POST['batchtext_FMC'];
    $array=explode("\r\n",trim($data));
    foreach ($array as $value) {
        $tmp=explode(" ",trim($value));
        $team=mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM currentFMC WHERE Name='".$tmp[1]."'"))['Team'];
        if($team==$tmp[0])
        releaseFMC($conn,$tmp[0],$tmp[1]);
        elseif($team=="")
        freesignFMC($conn,$tmp[0],$tmp[1],10);
        else
