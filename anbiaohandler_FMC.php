...
$suffix=$_GET['suffix'];
$teamsFMC=explode(' ',$_GET['turns']);
//$FMC_clubs_in_FML=array();
...
function checkanbiao($array,$str=""){
    ...
    if($pricesum+$array[$p][1]+(8-(count($r)+$playernum))*10>$money){
    ...
    ob_start();
    echo("<!DOCTYPE html><html><head><meta charset='utf-8'><title>球员暗标结果</title></head><body>");
    //读取FML暗标
for($i=0;$i<16;$i++){
    $file=$working_folder.strtoupper($teamsFMC[$i]).$suffix.".xlsx";
    if(!file_exists($file)){
        continue;
    }
